...
class Logger(object):
    def __init__(self, print_interval, backends, start_epoch, verbose=False):
        ...
        if (it % self.print_interval == 0):
            for b in self.backends:
                ...
                for n, m in [(n, m) for n, m in self.metrics.items() if m['level'] <= b.level and n.startswith('val') == val]:
                    ...
                    mv = m['meter'].get_val()
                    if mv is not None and isinstance(m['meter'], AverageMeter):
                        b.log_iteration_metric(n + '.avg', mv)
                    if mv is not None and isinstance(m['meter'], SumMeter):
                        b.log_iteration_metric(n + '.sum', mv)
...
class JsonBackend(object):
    ...
    def end(self):
        to_print = json.dump(self.json_log, open(self.filename, 'w'))
        if to_print is not None:
            print(to_print)


class StdOut1LBackend(object):
    def __init__(self, iters, val_iters, epochs, start_epoch, log_level=0):
        self.level = log_level
        self.iteration = 0
